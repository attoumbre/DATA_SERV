<?php
    ...
    function dossier_exist($mailSouscription){
        global $db;
        $e = array('mail' => $mailSouscription);
        $sql = 'SELECT * FROM souscription WHERE mailSouscription= :mail';
        $req = $db->prepare($sql);
        $req->execute($e);
...
//if ( htmlspecialchars(trim($_POST['email'])) != "") {
            ...
            //if($mailSouscription!=""){
                $vartest = dossier_exist($mailSouscription) ;
                $dateSou = date('d/m/Y') ;
                //echo $vartest ;
                if ( $vartest  == 0) {
